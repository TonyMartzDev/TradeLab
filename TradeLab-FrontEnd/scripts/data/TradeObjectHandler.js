        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains(this.storeName)) {
                    const store = db.createObjectStore(this.storeName, { keyPath: 'trade_id', autoIncrement: true });
                    
                    // Create indexes based on our schema
                    store.createIndex('tracked_account_id', 'tracked_account_id', { unique: false });
                    store.createIndex('trade_date', 'trade_date', { unique: false });
                    store.createIndex('ticker_symbol', 'ticker_symbol', { unique: false });
                }
            };
        });
    }
        ...
        if (this.storageType === 'indexeddb') {
            return new Promise((resolve, reject) => {
                ...
                if (criteria.tracked_account_id) {
                    ...
                    request = index.openCursor(IDBKeyRange.only(criteria.tracked_account_id));
